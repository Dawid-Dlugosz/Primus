...
import '../model/flashcard_ref.dart';
import '../model/flashcard_set.dart';
import '../model/learn_method.dart';
...
class FlashcardMainViewModel extends ChangeNotifier {
  ...
  Future<void> copyFlashcardSetToLearn() async {
    if (currentUser.toLearn != null &&
        currentUser.toLearn!.any(
            (element) => element.flashcardId == flashCardSet.flashcard.id)) {
      var toLearn = currentUser.toLearn!
          .where((element) => element.flashcardId == flashCardSet.flashcard.id)
          .first
          .copyWith(timeStamp: DateTime.now().millisecondsSinceEpoch);
      var indexToUpdate = currentUser.toLearn!.indexWhere(
          (element) => element.flashcardId == flashCardSet.flashcard.id);
              ...
              .map(
                (e) => ToLearnWord(
                  ...
                  learnMethod: LearnMethod(
                      flashcard: false, spelling: false, test: false),
                ),
              )
              .toList(),
        ),
    ...
    await FirebaseFirestore.instance
        .collection(FirebaseCollection.users.name)
        .doc(currentUser.uid)
        .update({'toLearn': updateToLearn.map((e) => e.toJson()).toList()});
  ...
  void setWords({setLoaded = false}) {
    ...
    if (currentUser.toLearn!
        .where((element) => element.flashcardId == reference.split('/')[1])
        .isEmpty) {
      return;
    }

    var words = currentUser.toLearn!
        .where((element) => element.flashcardId == reference.split('/')[1])
        .first
        .words;
    for (var element in words) {
      if (element.learnMethod.flashcard == true &&
          element.learnMethod.spelling == true &&
          element.learnMethod.test == true) {
